...
// https://www.youtube.com/watch?v=wB1pcXtEwIs for info on the physic engine (pos, vel, acc, speed, force)  
 ...
 class Particle {
    ...
    constructor() {
      ...
      this.prevPos = this.pos.copy(); // since the particle is a continious line, track its previous position.
    ...
    // track the x and y position of the particle everytime it moves
    updatePrev(){
    ...
    // assign vector (force) and color to the particles
    followMob() { 
      let x = floor(this.pos.x / scl);
      let y = floor(this.pos.y / scl);

      let index = x + y * cols; // calculate the index based on the position of the particle
   ...
   // assign vector (force) and color to the particles
    ...
    effect() {
     ...
     // let the collision happen only when the distance is less than 30
      if ( d < 30) {
        collision = true;
      } else {
        collision = false;
  ...
  let xoff = start; // restart xoff for every row
   for (let x = 0; x < cols; x++) {
    ...
    // assigning r, g, b values according to the noise
      ...
      let angle = (xoff, yoff, zoff) * PI; // the particles move in a semicircle (180 deg angle) based on their position in the 3d space
      let v = p5.Vector.fromAngle(angle); // vector starts from from angle
      /* noise is calculated in 3 dimensional space. (xoff and yoff are the positions, and zoff is treated as time 
      which means that each noise value is calculated based on a slice of the perlin noise space */
